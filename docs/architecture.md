...
Mikros training is curriculum-driven. `dataset build --profile mikros-curriculum-v1` emits prompt/completion examples with intent, expected mode, answer style, negative markers, and tags for smalltalk, coding, factual research, abstention, tool-use, and verifier-oriented repair. This dataset is for rebuilding the public Mikros artifact and for future SFT/DPO loops; it does not replace verifier gates.
