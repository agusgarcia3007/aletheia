...
Deployment is inference/research-only in public v1. `aletheia serve` loads one checkpoint and serves `/v1/models`, `/v1/chat/completions`, and `/v1/completions` behind local Bearer auth, plus `/readyz`, `/metrics`, and authenticated research job inspection. The Docker/Dokploy default is the single public checkpoint, `aletheia-mikros`; its chat endpoint uses deterministic routing, verified memory, and background research before falling back to model generation. Action-like chat requests are handled before retrieval: code snippets are generated by the checkpoint without web research, while repo repair is directed to `solve`. Deterministic chat responses are only a zero-step bootstrap fallback; trained checkpoints use the model. The API does not expose `solve`, verifiers, repository access, or command execution.
