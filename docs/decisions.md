...
## Milestone Honesty & De-Hardcoding V1

- Direction set: there is **no predefined large-model training target**. The parametric model stays minimal (smaller is better) and only does intent/routing/language; capability is meant to grow through the self-improving loop, retrieval, verifiers and memory — not by scaling weights. CLAUDE.md principle #5 was rewritten from "grow the model 35M→150M→360M" to "capability comes from the loop, not from parameters", and the Neural MoE note now states plainly that `TransformerV2` has **no backprop**, is not wired to training, and that the 360M MoE target is aspirational, not the plan.
- Honest abstention closed on the completions endpoint: `POST /v1/completions` now goes through `safeGenerate` like chat, so an untrained/bootstrap checkpoint (`Step <= 0`) returns the honest fallback instead of raw byte-model noise. Previously it called `s.generate` directly and could leak garbage — a real violation of principle #1.
- De-hardcoded synthesis: `internal/research/synthesis.go` no longer bakes in domain facts (it had a Copa América winners table and a literal "Alemania ganó el Mundial 2014, 1-0"). `CanonicalAnswer` is now a generic best-supported-sentence extractor — it lifts the answer verbatim from gathered evidence by query-token overlap, scaled by a length-aware threshold, and abstains (`ok=false`) when nothing meaningful is supported. No countries, tournaments or dates live in source anymore (principle #3: facts in storage, not in code).
- Eval fixtures follow suit: `evals/mikros_live` now seeds **natural-language** evidence and asserts the answer is extracted from it (not produced by a hardcoded synthesizer). The pure-synthesis `copa_last_five` template (which expected the baked "2024: Argentina; 2021: Argentina…" format) was removed and replaced with a neutral, non-sports `go_history` research case, proving the generic extractor works on any domain. `production` (0 hallucination / 0 false-verified / citation 1.0) and `mikros_artifact` (100%) stay clean; `mikros_live` VSR is unchanged at 0.9286 (the lone `nonsense_abstain` failure is pre-existing low-signal detection, not introduced here).
- Dead code removed: the `store_raw_html` config flag (declared and propagated but never implemented) and the write-only knowledge-graph nodes/edges in the research worker (`web_source`/`web_page`/`chunk`/`web_claim` were written but never read — web evidence is always recalled via SQL and the chunk retriever). The learning graph (router/selector/trajectory nodes) is untouched.
